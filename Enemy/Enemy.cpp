...
void Enemy::UpdatePath(const std::vector<std::vector<int>>& mapDistance) {
	...
	while (num != 0) {
		...
		for (auto& dir : PlayScene::directions) {
			int x = pos.x + dir.x;
			int y = pos.y + dir.y;
			if (x < 0 || x >= PlayScene::MapWidth || y < 0 || y >= PlayScene::MapHeight || mapDistance[y][x] != num - 1) continue;
...
void Enemy::Draw() const {
	Sprite::Draw();
	if (PlayScene::DebugMode) al_draw_circle(Position.x, Position.y, CollisionRadius, al_map_rgb(255, 0, 0), 2);
}
