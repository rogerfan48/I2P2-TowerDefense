...
namespace Engine {
	...
	void GameEngine::startEventLoop() {
		...
		while (!done) {
			...
			switch (event.type) {
			case ALLEGRO_EVENT_DISPLAY_CLOSE: 	// Event for clicking the window close button.
				LOG(VERBOSE) << "Window close button clicked";
				done = true;
				break;
			case ALLEGRO_EVENT_TIMER: 			// Event for redrawing the display.
				if (event.timer.source == update_timer) redraws++;
				break;
			...
			case ALLEGRO_EVENT_MOUSE_ENTER_DISPLAY:
				...
				break;
			default: 	// Ignore events that we're not interested in.
				break;
			}
